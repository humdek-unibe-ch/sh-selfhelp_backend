...
These two `supports.*` SemVer ranges are the **bidirectional gate** the registry resolver (`sh2-plugin-registry/scripts/resolve-core-candidate.mjs`) uses to refuse assembling an incompatible frontend+backend release. Pre-1.0 every `0.x` minor is breaking, so the ranges track one core minor (`<0.2.0`). `@selfhelp/shared` (typed contracts) is the complementary anchor for *response-shape* drift; the `release-manifest.json` ranges are the anchor for *version pairing*. Before finishing any frontend/backend change, re-check that both `supports.*` floors still reflect reality.

A coordinated breaking wave does **not** deadlock the registry: once both repos are tagged (in either order), the resolver pairs each new side against the counterpart's newest compatible git tag and stages both release PRs automatically — no manual publish. So the workflow for a coupled change is: bump both `supports.*` floors, tag both repos, merge the two staged PRs. Full rules: `docs/developer/cross-repo-compatibility-matrix.md`.
...
The navigation system is a cross-repo contract (backend model + `@selfhelp/shared` types/helpers + frontend/mobile renderers). Full reference: `docs/developer/29-navigation-menu-builder.md`.

- **Data model:** four system menus in `navigation_menus` (`web_header`, `web_footer`, `mobile_drawer`, `mobile_bottom_tabs`; key + platform + surface via lookups) with `id_preset`, `max_depth`, `item_limit`, `id_children_nav`, `show_breadcrumbs`, `show_pager`. Items live in `navigation_menu_items` (`item_type` lookup: `page` | `group` | `external_url` | `separator`; self-FK `id_parent_item`; `layer` `'top'`/NULL for double-header top row; per-item `id_children_nav` + `show_pager` overrides, NULL = inherit). Per-language presentation lives in `navigation_menu_item_translations` (`label`, `description`, `aria_label`). Page items keep `label` NULL — the page title drives the label; their translation rows are presentation-only (description/aria). Global settings (start pages, search mode, breadcrumb root, logo branding `logo_asset_path`/`logo_alt`/`id_logo_link_page`) live in `navigation_settings`.
- **Depth cap:** three levels (`NavigationMenuDepthSupport::MAX_LEVEL = 2`) enforced on every write path (create, reorder, bulk child add, bundle import). Bulk "include descendants" nests to the cap and flattens deeper pages. The stored `max_depth` is a depth-index render clamp (2 = all three levels), not the write cap.
- **Services:** `NavigationMenuService` builds the public `GET /navigation` payload (per access-type + language, cached via `CacheService` — call `invalidateNavigationCaches()` after ANY navigation write); `AdminNavigationService` is the admin CRUD + overview; `NavigationExportImportService` handles `selfhelp/navigation-bundle` v2.0 export/import; `NavigationAssignmentService` applies page-create menu assignments; support classes in `src/Navigation/` (`NavigationMenuDepthSupport`, `NavigationHeaderLayerSupport`, `NavigationMenuItemTranslationSupport`).
- **Payload contract:** menus always emit `key`/`platform`/`surface`/`preset`/`max_depth`/`item_limit`/`children_nav`/`show_breadcrumbs`/`show_pager`; items always emit every key (`label`, `description`, `aria_label`, `icon`, `mobile_icon`, `layer`, `children_nav`, `show_pager`, `page`, `children`, ...) with `null` for absent values. The payload also carries `branding` (`logo_url`, `logo_alt`, `link_url`) and `settings`. Never emit partial item shapes; update `config/schemas/api/v1/responses/frontend/get_navigation.json` + `@selfhelp/shared` types together.
- **Bundles:** `selfhelp/navigation-bundle` v2.0 only (v1.0 rejected). Items use `ref`/`parent_ref` + `page_keyword`; menus carry preset/depth/limit; optional embedded `pages[]`. The regression fixture is `tests/fixtures/examples/menu-demo.bundle.json` (canonical copy in `sh-selfhelp_frontend/examples/navigation/`) — keep both in sync and keep it importable (`NavigationBundleExportImportTest`).
- **When changing navigation behavior:** update entity + migration, JSON schemas, `NavigationMenuService` payload, admin service, bundle export/import + demo bundle, `@selfhelp/shared` types/helpers, web + mobile renderers, and the tests (`tests/Integration/CMS/Navigation*`, `tests/Golden/NavigationMenuBuilderWorkflowTest.php`) in the same wave, then raise the `supports.*` compatibility floors.
...
### CMS Style & Field Naming Rules

These govern CMS catalog **data values** (`styles.name`, `fields.name`,
`fieldType.name`), which are distinct from the database-object naming rules above
(those govern table/column/index identifiers). They are a cross-repo contract
shared with `@selfhelp/shared` (`STYLE_REGISTRY` + the `style_name`
discriminator), the frontend `BasicStyle` dispatcher / `FieldRenderer`, and the
mobile renderers, so a rename must land in every repo at once.
...
- Sections reference styles by `id_styles` (FK), so renaming a style name is a
  metadata change (an `UPDATE styles SET name = …` migration), not a content
  migration.
