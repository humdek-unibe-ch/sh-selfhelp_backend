...
class AdminSectionControllerTest extends BaseControllerTest
{
    ...
    public function testSectionLifecycleOnPage(): void
    {
        ...
        try {
            ...
            $this->client->request(
                ...
                (string) json_encode(['styleId' => self::DEFAULT_STYLE_ID_1, 'position' => 0])
            );
            $response = $this->client->getResponse();
            $this->assertSame(Response::HTTP_CREATED, $response->getStatusCode(), 'Failed to create section 1');
            $decodedResponseS1 = $this->decodeObject();
            ...
            $section1Id = $this->asInt($this->jsonGet($decodedResponseS1, 'data', 'id'));
            $this->assertGreaterThan(0, $section1Id, 'Section 1 ID is null');
            ...
            $this->client->request(
                ...
                [],
                ['HTTP_AUTHORIZATION' => 'Bearer ' . $token, 'CONTENT_TYPE' => 'application/json'],
                (string) json_encode(['styleId' => self::DEFAULT_STYLE_ID_2, 'position' => 1])
            );
            $response = $this->client->getResponse();
            $this->assertSame(Response::HTTP_CREATED, $response->getStatusCode(), 'Failed to create section 2');
            $decodedResponseS2 = $this->decodeObject();
            ...
            $section2Id = $this->asInt($this->jsonGet($decodedResponseS2, 'data', 'id'));
            $this->assertGreaterThan(0, $section2Id, 'Section 2 ID is null');
            ...
            $decodedResponseGet1 = $this->decodeObject();
            ...
            $sections = $this->asList($this->jsonGet($decodedResponseGet1, 'data', 'sections'));
            $this->assertCount(2, $sections, 'Incorrect number of sections after adding S1, S2');
            $this->assertSame($section1Id, $this->jsonGet($sections[0], 'id'), 'S1 not at position 0');
            $this->assertSame(0, $this->jsonGet($sections[0], 'position'), 'S1 position incorrect');
            $this->assertSame($section2Id, $this->jsonGet($sections[1], 'id'), 'S2 not at position 1');
            $this->assertSame(10, $this->jsonGet($sections[1], 'position'), 'S2 position incorrect');
            ...
            $decodedResponseS3 = $this->decodeObject();
            ...
            $section3Id = $this->asInt($this->jsonGet($decodedResponseS3, 'data', 'id'));
            $this->assertGreaterThan(0, $section3Id, 'Section 3 ID is null');
            ...
            $decodedResponseGet3 = $this->decodeObject();
            ...
            $sections = $this->asList($this->jsonGet($decodedResponseGet3, 'data', 'sections'));
            $this->assertCount(2, $sections, 'Incorrect number of top-level sections after adding S3');
            $this->assertSame($section1Id, $this->jsonGet($sections[0], 'id'), 'S1 is not the first section after S3 add');
            $this->assertCount(1, $this->asList($this->jsonGet($sections[0], 'children')), 'S1 does not have one child (S3) after S3 add');
            $this->assertSame($section3Id, $this->jsonGet($sections[0], 'children', 0, 'id'), 'S3 is not the child of S1 after S3 add');
            $this->assertSame($section2Id, $this->jsonGet($sections[1], 'id'), 'S2 is not the second section after S3 add');
            $this->assertEmpty($this->jsonGet($sections[1], 'children'), 'S2 should not have children after S3 add');
            ...
            $decodedResponseGet4 = $this->decodeObject();
            ...
            $sections = $this->asList($this->jsonGet($decodedResponseGet4, 'data', 'sections'));
            $this->assertCount(2, $sections, 'Incorrect number of top-level sections after removing S3');
            $this->assertSame($section1Id, $this->jsonGet($sections[0], 'id'), 'S1 not first after S3 removal');
            $this->assertEmpty($this->jsonGet($sections[0], 'children'), 'S1 should have no children after S3 removal');
            $this->assertSame($section2Id, $this->jsonGet($sections[1], 'id'), 'S2 not second after S3 removal');
            ...
            $decodedResponseGet5 = $this->decodeObject();
            ...
            $this->assertEmpty($this->jsonGet($decodedResponseGet5, 'data', 'sections'), 'Sections array not empty after deleting all sections');
    ...
    public function testMoveSectionBetweenParentsAndRejectInvalidMove(): void
    {
        ...
        try {
            ...
            $childId = $this->asInt($this->jsonGet($this->decodeObject(), 'data', 'id'));
            ...
            $this->client->request(
                ...
                [],
                ['HTTP_AUTHORIZATION' => 'Bearer ' . $token, 'CONTENT_TYPE' => 'application/json'],
                (string) json_encode(['childSectionId' => $childId, 'oldParentSectionId' => $p1, 'position' => 0])
            );
            $this->assertSame(
                Response::HTTP_OK,
                $this->client->getResponse()->getStatusCode(),
            ...
            $sections = $this->asList($this->jsonGet($this->decodeArray(), 'data', 'sections'));
            $byId = [];
            foreach ($sections as $sectionRaw) {
                $section = $this->asArray($sectionRaw);
                $byId[$this->asInt($section['id'])] = $section;
            }
            $this->assertArrayHasKey($p1, $byId, 'P1 missing from page sections');
            $this->assertArrayHasKey($p2, $byId, 'P2 missing from page sections');
            $p1ChildIds = array_column($this->asArray($byId[$p1]['children'] ?? []), 'id');
            $p2ChildIds = array_column($this->asArray($byId[$p2]['children'] ?? []), 'id');
            ...
            $this->client->request(
                ...
                [],
                ['HTTP_AUTHORIZATION' => 'Bearer ' . $token, 'CONTENT_TYPE' => 'application/json'],
                (string) json_encode(['childSectionId' => 999999])
            );
            $this->assertSame(
    ...
    private function createTopLevelSection(int $pageId, string $token, int $position): int
    {
        $this->client->request(
            ...
            [],
            ['HTTP_AUTHORIZATION' => 'Bearer ' . $token, 'CONTENT_TYPE' => 'application/json'],
            (string) json_encode(['styleId' => self::DEFAULT_STYLE_ID_1, 'position' => $position])
        );
        $this->assertSame(
            Response::HTTP_CREATED,
            $this->client->getResponse()->getStatusCode(),
        ...
        return $this->asInt($this->jsonGet($this->decodeObject(), 'data', 'id'));
    }
    ...
    public function testCreatePageSection(): void
    {
        ...
        $this->assertTrue(property_exists($dataData, 'id'), 'Response does not have section ID');
        $this->assertTrue(property_exists($dataData, 'position'), 'Response does not have position property');
    ...
    public function testCreateChildSection(): void
    {
        ...
        $this->client->request(
            ...
            [],
            ['HTTP_AUTHORIZATION' => 'Bearer ' . $token, 'CONTENT_TYPE' => 'application/json'],
            (string) json_encode($parentRequestData)
        );
        ...
        $parentData = $this->asObject(json_decode((string) $parentResponse->getContent()));
        $parentSectionId = $this->asInt($this->jsonGet($parentData, 'data', 'id'));
    ...
    public function testGetSectionWithFields(): void
    {
        ...
        $this->assertTrue(property_exists($data, 'data'), 'Response does not have data property');
        $dataData = $this->asObject($data->data);
        $this->assertTrue(property_exists($dataData, 'section'), 'Response data does not have section property');
        $this->assertTrue(property_exists($dataData, 'fields'), 'Response data does not have fields property');
        $this->assertTrue(property_exists($dataData, 'languages'), 'Response data does not have languages property');
        
        // Validate section data
        $section = $this->asObject($dataData->section);
        ...
        $sectionStyle = $this->asObject($section->style, 'Section style is null');
        $this->assertEquals(self::DEFAULT_STYLE_ID_1, $sectionStyle->id, 'Section style ID mismatch');
    ...
    public function testCreatePageSectionValidationErrors(): void
    {
        ...
        // Parse response data
        $data = $this->asObject(
            json_decode((string) $responseContent),
            'Response is not valid JSON: ' . (string) $responseContent
        );
        ...
        $validation = $this->asObject($data->validation);
        $this->assertTrue(property_exists($validation, 'errors'), 'Response does not have validation.errors property');
        $this->assertNotEmpty($validation->errors, 'No validation errors returned');
    }
    ...
    private function createTestSectionForChildTest(): void
    {
        ...
        if ($response->getStatusCode() === Response::HTTP_CREATED) {
            ...
            if ($data instanceof \stdClass && property_exists($data, 'data') && $data->data instanceof \stdClass && property_exists($data->data, 'id')) {
                 $this->testSectionId = $this->asInt($data->data->id);
            } else {
    ...
    public function testUpdateSection(): void
    {
        ...
        $sectionId = $this->asInt($this->jsonGet($createData, 'data', 'id'));
        ...
        try {
            ...
            $dataArr = $this->asArray($responseData['data']);
            $this->assertArrayHasKey('section', $dataArr);
            // Name should not have changed since we didn't provide sectionName
            $this->assertNotEquals('updated-test-section', $this->jsonGet($dataArr, 'section', 'name'));
            ...
            $getData = $this->decodeArray();
            // Verify the name hasn't changed
            $this->assertNotEquals('updated-test-section', $this->jsonGet($getData, 'data', 'section', 'name'));
    ...
    public function testUpdateSectionName(): void
    {
        ...
        $initialData = $this->decodeArray();
        $originalName = $this->jsonGet($initialData, 'data', 'section', 'name');
        ...
        try {
            ...
            $dataArr = $this->asArray($responseData['data']);
            $this->assertArrayHasKey('section', $dataArr);
            $this->assertSame('qa-updated-test-section-name', $this->jsonGet($dataArr, 'section', 'name'));
            ...
            $getData = $this->decodeArray();
            $this->assertSame('qa-updated-test-section-name', $this->jsonGet($getData, 'data', 'section', 'name'));
            $this->assertNotEquals($originalName, $this->jsonGet($getData, 'data', 'section', 'name'), 'Section name should have changed from original');
    ...
    public function testUpdateSectionWithInvalidFields(): void
    {
        ...
        $responseData = $this->decodeArray();
        $sectionId = $this->asInt($this->jsonGet($responseData, 'data', 'id'));
        ...
        try {
            ...
            $this->assertStringContainsString('do not belong to style', $this->asString($responseData['error']), 'Error message should mention invalid fields');
        } finally {
            // Clean up: delete the test section
            $this->client->request(
                'DELETE',
    ...
    public function testExportPageSections(): void
    {
        ...
        $this->client->request(
            ...
            [],
            ['HTTP_AUTHORIZATION' => 'Bearer ' . $token, 'CONTENT_TYPE' => 'application/json'],
            (string) json_encode(['styleId' => self::DEFAULT_STYLE_ID_1, 'position' => 1])
        );
        ...
        try {
            ...
            $this->assertArrayHasKey('data', $responseData);
            $dataArr = $this->asArray($responseData['data']);
            $this->assertArrayHasKey('sectionsData', $dataArr);
            
            // The export envelope wraps the section list under data.sectionsData.
            $sectionsData = $this->asList($dataArr['sectionsData']);
            ...
            // omitted when empty.
            foreach ($sectionsData as $sectionDataRaw) {
                $sectionData = $this->asArray($sectionDataRaw);
    ...
    public function testExportSection(): void
    {
        ...
        $createData = $this->decodeArray();
        $parentSectionId = $this->asInt($this->jsonGet($createData, 'data', 'id'));
        ...
        try {
            ...
            // Verify the minimized export shape (docs/section-export-import.md).
            $sectionData = $this->asArray($sectionsData[0]);
            $this->assertArrayHasKey('section_name', $sectionData);
            $this->assertArrayHasKey('style_name', $sectionData);
            if (array_key_exists('fields', $sectionData)) {
                $this->assertIsArray($sectionData['fields']);
    ...
    public function testImportSectionsToPage(): void
    {
        ...
        $this->client->request(
            ...
            [],
            ['HTTP_AUTHORIZATION' => 'Bearer ' . $token, 'CONTENT_TYPE' => 'application/json'],
            (string) json_encode(['sections' => $sectionsData])
        );
        ...
        $this->assertArrayHasKey('data', $responseData);
        $dataArr = $this->asArray($responseData['data']);
        $this->assertArrayHasKey('importedSections', $dataArr);
        $importedSections = $this->asList($dataArr['importedSections']);
        $this->assertNotEmpty($importedSections);
        
        $importedSection = $this->asArray($importedSections[0]);
        ...
        $this->assertStringStartsWith('qa-imported-test-section', $this->asString($importedSection['section_name']));
        
        // Clean up - delete the imported section
        $sectionId = $this->asInt($importedSection['id']);
        $this->client->request(
            'DELETE',
            sprintf('/cms-api/v1/admin/pages/%d/sections/%d', $pageId, $sectionId),
            [],
