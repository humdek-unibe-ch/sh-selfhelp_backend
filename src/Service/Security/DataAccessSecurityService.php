...
class DataAccessSecurityService
{
    ...
    /**
     * Apply filters to data based on permissions
     *
     * @param array $data Raw data to filter
     * @param array $permissions User permissions for resource type
    ...
    private function applyFilters(array $data, array $permissions, string $resourceType): array
    {
        ...
        return array_filter($data, function ($item) use ($permissionMap, $resourceType) {
            return $this->itemMatchesPermissions($item, $permissionMap, $resourceType);
        });
    }
    ...
    private function extractResourceId($item, string $resourceType): int
    {
        ...
        switch ($resourceType) {
            ...
            case LookupService::RESOURCE_TYPES_DATA_TABLE:
                ...
                return $item['table_id'] ?? $item['id'] ?? 0;

            case LookupService::RESOURCE_TYPES_PAGES:
                // For pages, the resource ID is the page ID
                return $item['id'] ?? 0;
